...
import { SmartAccount, Provider, types, utils, Wallet, ContractFactory } from "../../src";
...
describe("SmartAccount", async () => {
    ...
    describe("#transfer()", () => {
        ...
        it("should transfer ETH using paymaster to cover fee", async () => {
            ...
            const senderApprovalTokenBalanceBeforeTransfer = await account.getBalance(TOKEN);
            ...
            const senderApprovalTokenBalanceAfterTransfer = await account.getBalance(TOKEN);
            const receiverBalanceAfterTransfer = await provider.getBalance(RECEIVER);

            expect(paymasterBalanceBeforeTransfer - paymasterBalanceAfterTransfer >= BigInt(0)).to.be
                .true;
            expect(paymasterTokenBalanceAfterTransfer - paymasterTokenBalanceBeforeTransfer).to.be.equal(
                BigInt(minimalAllowance),
            );

            expect(senderBalanceBeforeTransfer - senderBalanceAfterTransfer).to.be.equal(BigInt(amount));
            expect(
    ...
    describe("#withdraw()", () => {
        ...
        it("should withdraw ETH to the L1 network using paymaster to cover fee", async () => {
            ...
            const l2BalanceBeforeWithdrawal = await account.getBalance();
            const l2ApprovalTokenBalanceBeforeWithdrawal = await account.getBalance(TOKEN);
            ...
            const l2BalanceAfterWithdrawal = await account.getBalance();
            const l2ApprovalTokenBalanceAfterWithdrawal = await account.getBalance(TOKEN);

            expect(paymasterBalanceBeforeWithdrawal - paymasterBalanceAfterWithdrawal >= BigInt(0)).to.be
                .true;
            expect(
                paymasterTokenBalanceAfterWithdrawal - paymasterTokenBalanceBeforeWithdrawal,
            ).to.be.equal(BigInt(minimalAllowance));

            expect(l2BalanceBeforeWithdrawal - l2BalanceAfterWithdrawal).to.be.equal(BigInt(amount));
            expect(
        ...
        it("should withdraw DAI to the L1 network", async () => {
            ...
            expect(result).not.to.be.null;
            expect(l2BalanceBeforeWithdrawal - l2BalanceAfterWithdrawal).to.be.equal(BigInt(amount));
            expect(l1BalanceAfterWithdrawal - l1BalanceBeforeWithdrawal).to.be.equal(BigInt(amount));
        }).timeout(25_000);
